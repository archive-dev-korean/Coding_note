...
- int() : 문자열을 정수로 변환, 다른 진법(2,8,16 진수 등)을 10진수로 변환 할 때 사용  
  - int(문자열, 진법)
  - int(문자열) -> 이럴 때는 숫자인 문자열만 사용가능
    - int('101',2) 이런식으로 적으면 이진수 문자열을 2진수로 읽어서 자동으로 변환됨
    - int('101',8) 이런식으로 쓰면 문자열을 8진수로 읽어서 8진법으로 변환됨
    - int('101',16) 이렇게 쓰면 16진법으로 변환됨
- bin() : 진법 변환 함수 입력값은 항상 10진수 이어야 함(무조건 정수 타입)
  - bin(정수) : 2진수로 바꿔줌 앞에 '0b10100'이런식으로 바뀐다 치면 0b는 2진수라는 표
- eval() : 문자열로 된 식을 그대로 코드처럼 실행함.
  - 치명적인 단점이 있는데 "__import__('os').system('rm -rf /')"이런식으로 문자열이 되어있으면 시스템 삭제가 되기 떄문에 실무에서는 잘 안쓴다고 함.
- set() : 집합을 만드는 메서드
  - 중복 제거됨
  - 순서 없음
  - 빠른 비교
    - 예를 들어 set('wpo') 이렇게 되면 {'w','p','o'}이렇게 출력됨 근데 w,p,o이렇게 될 수도 있고 p,o,w 이렇게 될수도 있고 o,p,w이런 순서로 출력될 수도 있음
    - 근데 ['s','p','o']이렇게 되어 있는 리스트를 set에 넣어도 {'s','p','o'} 이런 형태로 출력됨
    - 즉 문자열과 문자열을 가진 배열 모두 같은 형태로 출력됨
# 헷갈리는 사항
- 문자열은 불변
  ...
  - 인덱싱은 가능
  - 슬라이싱 가능
- 문자열 비교에서 in은 부분 문자열오 True 처리함
  - 예를 들어 'kim' in 'kim123' 이렇게 되어 있으면 'kim123'에 'kim'이 있기 때문에 True로 반환됨
  - 근데 대소문자는 구분됨 'kim' in 'Kim' 이렇게 쓰면 False로 반환됨
    - 그래서 정확한 비교를 쓸 떄는 '='쓰는걸 추천함.
